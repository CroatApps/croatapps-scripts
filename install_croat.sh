...
### TODO
#fer que coincideixi exactament el nom Linea:193 cat passwd OK fet
...
ops_ufw(){
    ...
    continue
    SSHDPORT1=$( grep -w Port /etc/ssh/sshd_config | awk '{print $2}' )
    SSHDPORT2=$( echo "$SSH_CLIENT" |  awk '{print $3}' )
    if [[ "$SSHDPORT1" == "$SSHDPORT2" ]]
    then
        SSHDPORT=${SSHDPORT1}
        ufw allow proto tcp from any to any port ${SSHDPORT}
        if [[  "$INSTALL_NODE" = "yes"  ]] ; then
            #[CROAT-NODE]
            ufw allow proto tcp from any to any port 46347
            ufw allow proto tcp from any to any port 46348
        fi
        if [[  "$INSTALL_WALLET" = "yes"  ]] ; then
            #[CROAT-WALLET]
            ufw allow proto tcp from any to any port 46349
        fi
        if [[  "$INSTALL_POOL" = "yes"  ]] ; then
            #[POOL]
            ufw allow "Nginx Full"
            ufw allow proto tcp from any to any port 8117
            ufw allow proto tcp from any to any port 3333
            ufw allow proto tcp from any to any port 4444
            ufw allow proto tcp from any to any port 5555
            ufw allow proto tcp from any to any port 7777
            ufw allow proto tcp from any to any port 9999
            #enable_ufw
        fi
    else
        echo WARNING: Can not find the port for SSHD
        #set_ports_manually
        #enable_ufw
    fi
}
...
load_colors
script_initializing
echo "script start in 20s, hit crtl+c for close this script"
sleep 20
check_usersudo
install_dependencies
get_vars
